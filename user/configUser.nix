{ lib, ... }: { options = {
	...
	userTitle = lib.mkOption {
		...
		default = "Sayid Munawar";

		# Description of the module.
		description = "The description (title) of the user. Can be overriden with `config.userTitle`.";
	};

	# Define the `userEmail` option.
	userEmail = lib.mkOption {
		# Set the type of the module (string).
		type = lib.types.str;

		# The default email of the user.
		default = "ayik@jogjacamp.co.id";

		# Description of the module.
		description = "The description (email) of the user. Can be overriden with `config.userEmail`.";
	};
